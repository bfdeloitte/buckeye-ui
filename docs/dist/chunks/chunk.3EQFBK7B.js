...
var SlSelect = class extends BuckeyeElement {
  constructor() {
    super(...arguments);
  ...
  getAllOptions() {
    return [...this.querySelectorAll("bui-option")];
  }
  // Gets the first <bui-option> element
  getFirstOption() {
    return this.querySelector("bui-option");
  }
      ...
      if (index < this.maxOptionsVisible || this.maxOptionsVisible <= 0) {
        return x`
                            <bui-tag
                              ...
                              size=${this.size}
                              removable
                              @bui-remove=${(event) => this.handleTagRemove(event, option)}
                            >
                              ${option.getTextLabel()}
                            </bui-tag>
                          `;
      } else if (index === this.maxOptionsVisible) {
        return x` <bui-tag size=${this.size}> +${this.selectedOptions.length - index} </bui-tag> `;
      } else {
        return null;
      }
    })}
