...
/**
 * Returns all form controls that are associated with the specified form. Includes both native and BuckeyeUI form
 * controls. Use this function in lieu of the `HTMLFormElement.elements` property, which doesn't recognize BuckeyeUI
 * form controls.
 */
export function getFormControls(form: HTMLFormElement) {
  ...
  const buckeyeFormControls = collection ? Array.from(collection) : [];

  // To return form controls in the right order, we sort by DOM index
  return [...formControls, ...buckeyeFormControls].sort((a: Element, b: Element) => {
