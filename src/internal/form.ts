...
export class FormControlController implements ReactiveController {
  ...
  private handleFormData(event: FormDataEvent) {
    ...
    const isButton = this.host.tagName.toLowerCase() === 'bui-button';
  ...
  /**
   * Dispatches a non-bubbling, cancelable custom event of type `bui-invalid`.
   * If the `bui-invalid` event will be cancelled then the original `invalid`
   * event (which may have been passed as argument) will also be cancelled.
   * If no original `invalid` event has been passed then the `bui-invalid`
   * event will be cancelled before being dispatched.
   */
  emitInvalidEvent(originalInvalidEvent?: Event) {
    const slInvalidEvent = new CustomEvent<Record<PropertyKey, never>>('bui-invalid', {
      bubbles: false,
      composed: false,
      cancelable: true,
      detail: {}
