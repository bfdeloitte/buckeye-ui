...
import type InputEvent from '../../events/bui-input';
 ...
 * @since 2.0
 *
 * @dependency bui-button
 * @dependency bui-button-group
 * @dependency bui-dropdown
 * @dependency bui-input
 * @dependency bui-visually-hidden
 *
 * @slot label - The color picker's form label. Alternatively, you can use the `label` attribute.
 *
 * @event bui-blur - Emitted when the color picker loses focus.
 * @event bui-change - Emitted when the color picker's value changes.
 * @event bui-focus - Emitted when the color picker receives focus.
 * @event bui-input - Emitted when the color picker receives input.
 ...
 */
@customElement('bui-color-picker')
export default class ColorPicker extends BuckeyeElement implements BuckeyeFormControl {
  ...
  @query('[part~="input"]') input: Input;
  @query('.color-dropdown') dropdown: Dropdown;
  ...
  private handleInputChange(event: ChangeEvent) {
    ...
    // Prevent the <bui-input>'s bui-change event from bubbling up
  ...
  private handleInputInput(event: InputEvent) {
    ...
    // Prevent the <bui-input>'s bui-input event from bubbling up
    event.stopPropagation();
  }
  ...
  reportValidity() {
    if (!this.inline && !this.validity.valid) {
      ...
      if (!this.disabled) {
        // By standards we have to emit a `bui-invalid` event here synchronously.
