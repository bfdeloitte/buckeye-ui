...
describe('<bui-tree-item>', () => {
  let leafItem: TreeItem;
  let parentItem: TreeItem;

  beforeEach(async () => {
    leafItem = await fixture(html` <bui-tree-item>Node 1</bui-tree-item> `);
  ...
  describe('when the user clicks the expand button', () => {
    describe('and the item is collapsed', () => {
      it('should emit bui-expand and bui-after-expand events', async () => {
        // Arrange
        const expandSpy = sinon.spy();
        const afterExpandSpy = sinon.spy();

        parentItem.addEventListener('bui-expand', expandSpy);
        parentItem.addEventListener('bui-after-expand', afterExpandSpy);
    ...
    describe('and the item is expanded', () => {
      it('should emit bui-collapse and bui-after-collapse events', async () => {
        // Arrange
        const collapseSpy = sinon.spy();
        const afterCollapseSpy = sinon.spy();

        parentItem.addEventListener('bui-collapse', collapseSpy);
        parentItem.addEventListener('bui-after-collapse', afterCollapseSpy);

        parentItem.expanded = true;
        await oneEvent(parentItem, 'bui-after-expand');
  ...
  describe('when the item is lazy', () => {
    it('should emit bui-lazy-change when the lazy attribute is added and removed', async () => {
      // Arrange
      const lazyChangeSpy = sinon.spy();

      parentItem.addEventListener('bui-lazy-change', lazyChangeSpy);
