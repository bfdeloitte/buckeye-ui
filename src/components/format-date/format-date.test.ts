...
import type FormatDate from './format-date';

describe('<bui-format-date>', () => {
  describe('defaults ', () => {
    ...
    it('default properties', async () => {
      const el = await fixture<FormatDate>(html` <bui-format-date></bui-format-date> `);
  ...
  describe('lang property', () => {
    ...
    results.forEach(setup => {
      it(`date has correct language format: ${setup.lang}`, async () => {
        const el = await fixture<FormatDate>(
          html`
            <bui-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" lang="${setup.lang}"></bui-format-date>
          `
        );
  ...
  describe('era property', () => {
    ...
    eras.forEach((eraFormat: 'narrow' | 'short' | 'long') => {
      it(`date has correct era format: ${eraFormat}`, async () => {
        const el = await fixture<FormatDate>(
          html`
            <bui-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" era="${eraFormat}"></bui-format-date>
          `
        );
  ...
  describe('year property', () => {
    ...
    yearFormats.forEach((yearFormat: 'numeric' | '2-digit') => {
      it(`date has correct year format: ${yearFormat}`, async () => {
        const el = await fixture<FormatDate>(
          html`
            <bui-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" year="${yearFormat}"></bui-format-date>
          `
        );
  ...
  describe('day property', () => {
    ...
    dayFormats.forEach((dayFormat: 'numeric' | '2-digit') => {
      it(`date has correct day format: ${dayFormat}`, async () => {
        const el = await fixture<FormatDate>(
          html`
            <bui-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" day="${dayFormat}"></bui-format-date>
          `
        );
  ...
  describe('hour property', () => {
    ...
    hourFormats.forEach((hourFormat: 'numeric' | '2-digit') => {
      it(`date has correct hour format: ${hourFormat}`, async () => {
        const el = await fixture<FormatDate>(
          html`
            <bui-format-date .date="${new Date(new Date().getFullYear(), 0, 1)}" hour="${hourFormat}"></bui-format-date>
          `
        );
