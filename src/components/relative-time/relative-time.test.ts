...
interface RelativeTimeTestCase {
  date: Date;
  expectedOutput: string;
}

const extractTimeElement = (relativeTime: RelativeTime): HTMLTimeElement | null => {
  return relativeTime.shadowRoot?.querySelector('time') || null;
};

const expectFormattedRelativeTimeToBe = async (relativeTime: RelativeTime, expectedOutput: string): Promise<void> => {
...
const createRelativeTimeWithDate = async (relativeDate: Date): Promise<RelativeTime> => {
  const relativeTime: RelativeTime = await fixture<RelativeTime>(
    html` <bui-relative-time lang="en-US"></bui-relative-time> `
  );
  relativeTime.date = relativeDate;
  return relativeTime;
};

const expectTitleToMatchLocalizedTimeString = (relativeTime: RelativeTime) => {
...
const testCases: RelativeTimeTestCase[] = [
  {
...
describe('bui-relative-time', () => {
  ...
  describe('handles time correctly', () => {
    ...
    testCases.forEach(testCase => {
      ...
      it(`shows the correct relative time given a String object: ${testCase.expectedOutput}`, async () => {
        ...
        const relativeTime: RelativeTime = await fixture<RelativeTime>(
          html` <bui-relative-time lang="en-US" date="${dateString}"></bui-relative-time> `
        );
    ...
    it('always shows numeric if requested via numeric property', async () => {
      const relativeTime: RelativeTime = await fixture<RelativeTime>(
        html` <bui-relative-time lang="en-US" numeric="always"></bui-relative-time> `
      );
    ...
    it('shows human readable form if appropriate and numeric property is auto', async () => {
      const relativeTime: RelativeTime = await fixture<RelativeTime>(
        html` <bui-relative-time lang="en-US" numeric="auto"></bui-relative-time> `
      );
      relativeTime.date = yesterday;
    ...
    it('allows to use a short form of the unit', async () => {
      ...
      const relativeTime: RelativeTime = await fixture<RelativeTime>(
        html` <bui-relative-time lang="en-US" numeric="always" format="short"></bui-relative-time> `
      );
    ...
    it('allows to use a long form of the unit', async () => {
      const twoYearsAgo = new Date(currentTime.getTime() - 2 * nonLeapYearInSeconds);
      const relativeTime: RelativeTime = await fixture<RelativeTime>(
        html` <bui-relative-time lang="en-US" numeric="always" format="long"></bui-relative-time> `
      );
      relativeTime.date = twoYearsAgo;
    ...
    it('is formatted according to the requested locale', async () => {
      const relativeTime: RelativeTime = await fixture<RelativeTime>(
        html` <bui-relative-time lang="de-DE" numeric="auto"></bui-relative-time> `
      );
      relativeTime.date = yesterday;
  ...
  it('does not display a time element on invalid time string', async () => {
    ...
    const relativeTime: RelativeTime = await fixture<RelativeTime>(
      html` <bui-relative-time lang="en-US" date="${invalidDateString}"></bui-relative-time> `
    );
