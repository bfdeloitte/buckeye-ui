...
import type AnimatedImage from './animated-image';

describe('<bui-animated-image>', () => {
  it('should render a component', async () => {
    const animatedImage = await fixture(html` <bui-animated-image></bui-animated-image> `);
  ...
  files.forEach((file: string) => {
    it(`should load a ${file} without errors`, async () => {
      const animatedImage = await fixture<AnimatedImage>(html` <bui-animated-image></bui-animated-image> `);
      let errorCount = 0;
      oneEvent(animatedImage, 'bui-error').then(() => errorCount++);
  ...
  it('should emit an error event on invalid url', async () => {
    ...
    const errorPromise = oneEvent(animatedImage, 'bui-error');
    animatedImage.src = 'completelyWrong';

    await errorPromise;
  });
});
async function loadImage(animatedImage: AnimatedImage, file: string) {
  const loadingPromise = oneEvent(animatedImage, 'bui-load');
