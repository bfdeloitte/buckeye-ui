...
describe('<bui-icon-button>', () => {
  describe('defaults ', () => {
    it('default properties', async () => {
      const el = await fixture<IconButton>(html` <bui-icon-button></bui-icon-button> `);
  ...
  describe('when styling the host element', () => {
    it('renders the correct color and font size', async () => {
      ...
      const icon = el.shadowRoot!.querySelector('bui-icon')!;
  ...
  describe('when icon attributes are present', () => {
    it('renders an bui-icon from a library', async () => {
      const el = await fixture<IconButton>(html` <bui-icon-button library="system" name="check"></bui-icon-button> `);
      expect(el.shadowRoot?.querySelector('bui-icon')).to.exist;
    });

    it('renders an bui-icon from a src', async () => {
      const fakeId = 'test-src';
      const el = await fixture<IconButton>(html` <bui-icon-button></bui-icon-button> `);

      el.src = `data:image/svg+xml,${encodeURIComponent(`<svg id="${fakeId}"></svg>`)}`;

      const internalIcon = el.shadowRoot?.querySelector('bui-icon');

      await waitUntil(() => internalIcon?.shadowRoot?.querySelector('svg'), 'SVG not rendered');

      expect(internalIcon).to.exist;
      expect(internalIcon?.shadowRoot?.querySelector('svg')).to.exist;
      expect(internalIcon?.shadowRoot?.querySelector('svg')?.getAttribute('id')).to.equal(fakeId);
    });
  });

  describe('when href is present', () => {
    it('renders as an anchor', async () => {
      const el = await fixture<IconButton>(html` <bui-icon-button href="some/path"></bui-icon-button> `);
    ...
    describe('and target is present', () => {
      ['_blank', '_parent', '_self', '_top'].forEach((target: LinkTarget) => {
        it(`the anchor target is the provided target: ${target}`, async () => {
          const el = await fixture<IconButton>(
            html` <bui-icon-button href="some/path" target="${target}"></bui-icon-button> `
          );
    ...
    describe('and download is present', () => {
      it(`the anchor download attribute is the provided download`, async () => {
        ...
        const el = await fixture<IconButton>(
          html` <bui-icon-button href="some/path" download="${fakeDownload}"></bui-icon-button> `
        );
  ...
  describe('when label is present', () => {
    it('the internal aria-label attribute is set to the provided label when rendering a button', async () => {
      ...
      const el = await fixture<IconButton>(html` <bui-icon-button label="${fakeLabel}"></bui-icon-button> `);
    ...
    it('the internal aria-label attribute is set to the provided label when rendering an anchor', async () => {
      const fakeLabel = 'some label';
      const el = await fixture<IconButton>(
        html` <bui-icon-button href="some/path" label="${fakeLabel}"></bui-icon-button> `
      );
  ...
  describe('when disabled is present', () => {
    it('the internal button has a disabled attribute when rendering a button', async () => {
      const el = await fixture<IconButton>(html` <bui-icon-button disabled></bui-icon-button> `);
      expect(el.shadowRoot?.querySelector(`button[disabled]`)).to.exist;
    });

    it('the internal anchor has an aria-disabled attribute when rendering an anchor', async () => {
      const el = await fixture<IconButton>(html` <bui-icon-button href="some/path" disabled></bui-icon-button> `);
