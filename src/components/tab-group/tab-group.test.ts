...
import type TabShowEvent from '../../events/bui-tab-show';
...
const waitForScrollButtonsToBeRendered = async (tabGroup: TabGroup): Promise<void> => {
  await waitUntil(() => {
    const scrollButtons = tabGroup.shadowRoot?.querySelectorAll('bui-icon-button');
    return scrollButtons?.length === 2;
  });
};

const getClientRectangles = (tabGroup: TabGroup): ClientRectangles => {
...
const expectHeaderToBeVisible = (container: HTMLElement, dataTestId: string): void => {
  const generalHeader = queryByTestId<Tab>(container, dataTestId);
...
const expectOnlyOneTabPanelToBeActive = async (container: HTMLElement, dataTestIdOfActiveTab: string) => {
  await waitUntil(() => {
    const tabPanels = Array.from(container.getElementsByTagName('bui-tab-panel'));
    const activeTabPanels = tabPanels.filter((element: TabPanel) => element.hasAttribute('active'));
...
const expectPromiseToHaveName = async (showEventPromise: Promise<TabShowEvent>, expectedName: string) => {
  const showEvent = await showEventPromise;
  expect(showEvent.detail.name).to.equal(expectedName);
};

const waitForHeaderToBeActive = async (container: HTMLElement, headerTestId: string): Promise<Tab> => {
  const generalHeader = queryByTestId<Tab>(container, headerTestId);
  await waitUntil(() => {
...
describe('<bui-tab-group>', () => {
  ...
  it('displays all tabs', async () => {
    const tabGroup = await fixture<TabGroup>(html`
      <bui-tab-group>
        <bui-tab slot="nav" panel="general" data-testid="general-tab-header">General</bui-tab>
        <bui-tab slot="nav" panel="disabled" disabled data-testid="disabled-tab-header">Disabled</bui-tab>
        <bui-tab-panel name="general">This is the general tab panel.</bui-tab-panel>
  ...
  it('shows the first tab to be active by default', async () => {
    const tabGroup = await fixture<TabGroup>(html`
      <bui-tab-group>
        ...
        <bui-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</bui-tab-panel>
  ...
  describe('scrolling behavior', () => {
    const generateTabs = (n: number): HTMLTemplateResult[] => {
      ...
      for (let i = 0; i < n; i++) {
        result.push(html`<bui-tab slot="nav" panel="tab-${i}">Tab ${i}</bui-tab>
          <bui-tab-panel name="tab-${i}">Content of tab ${i}0</bui-tab-panel> `);
      }
      return result;
    };
    ...
    it('shows scroll buttons on too many tabs', async () => {
      const tabGroup = await fixture<TabGroup>(html`<bui-tab-group> ${generateTabs(30)} </bui-tab-group>`);
    ...
    it('does not show scroll buttons if all tabs fit on the screen', async () => {
      const tabGroup = await fixture<TabGroup>(html`<bui-tab-group> ${generateTabs(2)} </bui-tab-group>`);
    ...
    it('does not show scroll buttons if placement is start', async () => {
      const tabGroup = await fixture<TabGroup>(html`<bui-tab-group> ${generateTabs(50)} </bui-tab-group>`);
      tabGroup.placement = 'start';
    ...
    it('does scroll on scroll button click', async () => {
      ...
      const tabGroup = await fixture<TabGroup>(
        html`<bui-tab-group> ${generateTabs(numberOfElements)} </bui-tab-group>`
      );
      ...
      const scrollToRightButton = tabGroup.shadowRoot?.querySelector('bui-icon-button[part*="scroll-button--end"]');
  ...
  describe('tab selection', () => {
    const expectCustomTabToBeActiveAfter = async (tabGroup: TabGroup, action: () => Promise<void>): Promise<void> => {
      const generalHeader = await waitForHeaderToBeActive(tabGroup, 'general-header');
      generalHeader.focus();

      const customHeader = queryByTestId<Tab>(tabGroup, 'custom-header');
      expect(customHeader).not.to.have.attribute('active');

      const showEventPromise = oneEvent(tabGroup, 'bui-tab-show') as Promise<TabShowEvent>;
      await action();
    ...
    const expectGeneralTabToBeStillActiveAfter = async (
      ...
      oneEvent(tabGroup, 'bui-tab-show').then(() => (showEventFired = true));
      oneEvent(tabGroup, 'bui-tab-hide').then(() => (hideEventFired = true));
      await action();
    ...
    it('selects a tab by clicking on it', async () => {
      const tabGroup = await fixture<TabGroup>(html`
        <bui-tab-group>
          <bui-tab slot="nav" panel="general" data-testid="general-header">General</bui-tab>
          <bui-tab slot="nav" panel="custom" data-testid="custom-header">Custom</bui-tab>
          <bui-tab-panel name="general">This is the general tab panel.</bui-tab-panel>
          <bui-tab-panel name="custom" data-testid="custom-tab-content">This is the custom tab panel.</bui-tab-panel>
        </bui-tab-group>
      `);
    ...
    it('does not change if a disabled tab is clicked', async () => {
      const tabGroup = await fixture<TabGroup>(html`
        <bui-tab-group>
          <bui-tab slot="nav" panel="general" data-testid="general-header">General</bui-tab>
          <bui-tab slot="nav" panel="disabled" data-testid="disabled-header" disabled>disabled</bui-tab>
          <bui-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</bui-tab-panel>
          <bui-tab-panel name="disabled">This is the disabled tab panel.</bui-tab-panel>
        </bui-tab-group>
      `);
    ...
    it('does not allow selection of disabled tabs with arrow keys', async () => {
      const tabGroup = await fixture<TabGroup>(html`
        <bui-tab-group>
          ...
          <bui-tab-panel name="general" data-testid="general-tab-content">This is the general tab panel.</bui-tab-panel>
          <bui-tab-panel name="disabled">This is the custom tab panel.</bui-tab-panel>
        </bui-tab-group>
      `);
