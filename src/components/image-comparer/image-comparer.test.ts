...
import type ImageComparer from './image-comparer';

describe('<bui-image-comparer>', () => {
  ...
  it('should emit change event when position changed manually', async () => {
    ...
    el.addEventListener('bui-change', handler, { once: true });
  ...
  it('should set position by attribute', async () => {
    const el = await fixture<ImageComparer>(html`
      <bui-image-comparer position="10">
        <div slot="before"></div>
        <div slot="after"></div>
      </bui-image-comparer>
    `);
